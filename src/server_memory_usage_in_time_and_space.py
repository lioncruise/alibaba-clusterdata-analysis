...
if __name__ == '__main__':
    ...
    try:
        ...
        df = df.pivot(index='machine_id', columns='timestamp', values='memory_usage')
        ...
        plt.colorbar(im, fraction=0.015, pad=0.05).ax.set_ylabel('memory usage (%)')
        plt.title('server memory usage in time and space')
        plt.xlabel('time (h)')
